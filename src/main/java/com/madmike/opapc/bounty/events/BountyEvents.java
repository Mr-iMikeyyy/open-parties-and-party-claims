            ...
            if (entity instanceof ServerPlayer player) {
                if (damageSource.getEntity() instanceof ServerPlayer killer) {
                    if (OPAPCComponents.BOUNTY.get(player).getBounty() > 0) {
                        ModComponents.CURRENCY.get(killer).modify(OPAPCComponents.BOUNTY.get(player).getBounty() * 10000L);
                        OPAPCComponents.BOUNTY.get(player).setBounty(0);
                    }
                    else {
                        OPAPCComponents.BOUNTY.get(killer).setBounty(OPAPCComponents.BOUNTY.get(killer).getBounty + 10);
                    }
                }
            }
